...
class CDAatracproject {
    ...
    async init(cmiImageURL: string) {
        this.initialRAM = parseImage(new Uint8Array(await (await fetch(cmiImageURL)).arrayBuffer()));
...
class VM {
    ...
    async init(links: Links) {
        self.importScripts(links.emuWorker);
        // @ts-ignore
        this.emulator = new V86({
            wasm_path: links.emulator,
        ...
        await this.linker.init(links.cmi);
...
if (typeof WorkerGlobalScope !== 'undefined' && self instanceof WorkerGlobalScope) {
    ...
    onmessage = async (ev) => {
        ...
        if (action === 'init') {
            atrac3vm = new VM();
            await atrac3vm.init(others.links);
